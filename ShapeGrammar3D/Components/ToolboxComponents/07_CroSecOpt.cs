...
[System.Obsolete("Archived component: not used by the referenced Grasshopper definitions. Hidden from the toolbar.", false)]
        public class ST_CroSecOpt : GH_Component
    {
        ...
        {
            pManager.RegisterParam(new Param_TB_Model(), GH_ParamAccess.item);
        ...
        {
            // --- variables ---
            GH_TB_Model gh_m = new GH_TB_Model();
            ...
            // // copy the model
            TB_Model mdl = Common.DeepCopy(gh_m.Value);
            ...
            List<TB_Element_1D> new_elems = new List<TB_Element_1D>();
            foreach (TB_Element_1D e in mdl.Elem1Ds)
            {
                ...
                {
                    new_elems.Add(new TB_Element_1D(e.Line, e.Tag, gh_secs[0].Value, e.Vz, e.Buckling_Length));
                }
                else
                {
                    new_elems.Add(new TB_Element_1D(e.Line, e.Tag, e.Sec, e.Vz, e.Buckling_Length));
                }
            }

            mdl = new TB_Model(new_elems, mdl.Sups, mdl.Loads);
            SolveLS slv = new SolveLS(ref mdl);
            mdl = slv.Mdl;

            // // calculate buckling length
            foreach (TB_Element_1D rel_e in new_elems)
            {
            ...
            while(cnt_iter < max_iter)
            {
                ...
                List<TB_Element_1D> updated_elems = new List<TB_Element_1D>();
                foreach (TB_Element_1D e in mdl.Elem1Ds)
                {
                    updated_elems.Add(new TB_Element_1D(e.Line, e.Tag, e.Sec, e.Vz, e.Buckling_Length));
                }

                mdl = new TB_Model(updated_elems, mdl.Sups, mdl.Loads);
            ...
            // --- output ---
            DA.SetData(0, new GH_TB_Model(mdl));
            DA.SetData(1, mdl.Weight);
            DA.SetData(2, cnt_iter);

        }

        private void UpdateBucklingLength(TB_Element_1D _e)
        {
        ...
        protected override System.Drawing.Bitmap Icon
        {
            get
            {
                ...
                return Properties.Resources.icons_C_Sol_CSOpt;
            }
        }
        ...
        {
            get { return new Guid("3732df1f-6e4b-4405-a3a5-c83195dc1c0b"); }
        }
    }
}