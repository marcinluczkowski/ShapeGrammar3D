...
namespace ShapeGrammar3D.Components.ToolboxComponents
{
    public class ST_Assembly : GH_Component
    {
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            List<TB_Element_1D> elem1Ds = gh_elem1Ds.Select(x => x.Value).ToList();
            List<TB_Support> sups = gh_sups.Select(x => x.Value).ToList();
            List<TB_Load> loads = gh_loads.Select(x => x.Value).ToList();
