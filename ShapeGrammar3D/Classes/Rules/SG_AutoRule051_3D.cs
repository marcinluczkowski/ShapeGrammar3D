    ...
    public class SG_AutoRule051_3D : SG_Rule
    {
        ...
        public override string RuleOperation(ref SG_Shape ss_ref, ref SG_Genotype gt)
        {
            ...
            for (int i = 0; i < selectedIntGenes.Count; i++)
            {
                ...
                {
                    var stud = (SG_Elem1D)studElements[j]; //.Nodes[0].Elements.Where(e => e.Autorule == UT.RULE010_MARKER).ToList()[0]); //250916
                ...
                if (closestStuds.Count > 2)
                {
                    ...
                    //251002
                    newElem0 = new SG_Elem1D(new Line(closestStuds[0].Pt, closestStuds[1].Pt), -999, "3DAR5", new SH_CrossSection_Beam()) { Autorule = UT.RULE051_MARKER };
                    newElem1 = new SG_Elem1D(new Line(closestStuds[0].Pt, closestStuds[2].Pt), -999, "3DAR5", new SH_CrossSection_Beam()) { Autorule = UT.RULE051_MARKER };
