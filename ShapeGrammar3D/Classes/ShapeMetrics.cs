...
namespace ShapeGrammar3D.Classes
{
    ...
    public static class ShapeMetrics
    {
        ...
        /// <summary>13 – Convex Hull Volume. Calculate envelope volume generated by Grasshopper's convex hull voulume.</summary>
        public static double ConvexHullVolume(SG_Shape shape)
        {
            if (shape?.Elems == null || shape.Elems.Count == 0) return 0.0;
            if (shape.Nodes == null || shape.Nodes.Count < 4) return 0.0; // Need at least 4 points for 3D volume

            try
            {
                // Collect all unique 3D points from nodes
                var pts = shape.Nodes
                    .Where(n => n?.Pt != null)
                    .Select(n => n.Pt)
                    .Distinct()
                ...
                if (pts.Count < 4) return 0.0;

                // Compute 3D convex hull using Grasshopper
                var hull = Grasshopper.Kernel.Geometry.ConvexHull.Solver.ComputeHull((IEnumerable<Grasshopper.Kernel.Types.GH_Point>)pts);

                if (hull == null || hull.Count < 4)
                    return 0.0;

                // Create mesh manually from convex hull points
                var mesh = new Mesh();
                ...
                // Create faces from hull edges to centroid
