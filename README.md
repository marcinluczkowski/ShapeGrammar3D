...
Define the design domain, supports, loads, materials, sections, and grammar rules in Grasshopper.



Assemble an initial SG_Shape or create one through SG_AutoRule_InitShape_3D.



Configure optimization and clustering parameters using GI Settings.



Run a grammar interpreter such as GI_FromSgShape, GI_LargeSg, or GI_Large From SG_Shape.



The interpreter evaluates each individual by:
...
The optimizer updates the population using SG_GA or SG_MOGA (NSGA-II).



The population is clustered by weighted topology, shape, and optional fitness descriptors.



Results are output as assemblies, JSON runs, Pareto fronts, convergence data, and preview geometry.
...
Material definitions for structural analysis.
...
Cross-section definitions and presets.
...
1D element creation from lines or curves.
...
Point, line, and curve load components.
...
Assembly of elements, supports, loads, and initial shapes.
...
Manual and automatic grammar rules.
...
Grammar interpreters, optimization settings, JSON readers, metric sweeps.
...
Pareto, radar, convergence, optimization, force, deformation, and table previews.
...
Core structural graph containing nodes, elements, supports, loads, and boundary data.
...
Base class for shape grammar operations.
...
Encodes integer and continuous genes used by grammar rules.
...
Finite element representation assembled from an SG_Shape.
...
Direct-stiffness linear static solver.
...
Applies rules, builds models, solves structures, and computes objectives/descriptors.
...
Single-objective genetic algorithm with clustering support.
...
Multi-objective optimizer using NSGA-II principles.
...
Catalogue of graph/topology descriptors.
...
Catalogue of geometric/shape descriptors.
...
Computes normalized feasibility violation metrics.
...
Streams large optimization runs to JSON.
...
Lightweight parsed result bundle for preview components.





SGShapeGrammar3DAssembly



Stores generated shapes, models, individuals, and run metadata.
